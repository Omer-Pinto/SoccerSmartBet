...
| U9 | League page missed in dashboard redesign — added merged rows, sort, paging, calendar, split stats, evaluated-only filter | d11b3ea | Replaced 9-column flat layout with merged-row game-groups mirroring team.html; onSortClick calls sortGroups(buildGroups(_rawBets)) so sort actually re-orders rows. |
